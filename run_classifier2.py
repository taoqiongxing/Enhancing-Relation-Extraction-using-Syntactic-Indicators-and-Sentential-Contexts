...
def convert_single_example(ex_index, example, label_list, max_seq_length,
  ...
  e11_p = tokens_a.index("##11") + 2  ##entity1 的首位置+1+1
  e12_p = tokens_a.index("##12") - 1  ##entity1 的尾位置-2+1
  e21_p = tokens_a.index("##21") + 2  ##entity2 的首位置
  e22_p = tokens_a.index("##22") - 1  ##entity2 的尾位置

  #si_id = len(tokens_a) + 1
...
def create_model(bert_config, is_training, input_ids,
  ...
  output_bias = tf.get_variable(
      "output_bias", [num_labels+3], initializer=tf.zeros_initializer())
  ...
  with tf.variable_scope("loss"):
    ...
    logits = tf.concat([tf.reduce_sum(logits[:,:4], axis=-1, keep_dims=True), logits[:,4:]], axis=-1)
    ...
    otherloss = - one_hot_labels[:,0] * log_probs[:,0]

    l2 = tf.add_n([tf.nn.l2_loss(v) for v in tf.trainable_variables()])

    rc_probabilities = probabilities - probabilities * one_hot_labels
    second_pre = - tf.reduce_max(rc_probabilities, axis=-1) + 1
    rc_loss = - tf.math.log(second_pre)

    loss = tf.reduce_mean(per_example_loss) + 5 * tf.reduce_mean(rc_loss) + l2 * l2_reg_lambda
...
'''
 with tf.variable_scope("loss"):
    ...
    logits = tf.matmul(output_layer, output_weights, transpose_b=True)
    logits = tf.nn.bias_add(logits, output_bias)
    logits = tf.concat([logits[:,0:1]*2, logits[:,1:]], axis=1)
    probabilities = tf.nn.softmax(logits, axis=-1)
    log_probs = tf.nn.log_softmax(logits, axis=-1)
    ...
    per_example_loss_plus = -tf.reduce_sum(one_hot_labels[:,1:] * log_probs[:,1:], axis=-1)

    l2 = tf.add_n([tf.nn.l2_loss(v) for v in tf.trainable_variables()])

    loss = tf.reduce_mean(per_example_loss) + l2 * l2_reg_lambda + tf.reduce_mean(per_example_loss_plus)

    rc_probabilities = (probabilities - probabilities * one_hot_labels)
    second_pre = tf.reduce_sum(rc_probabilities, axis=-1) \
    
                   - tf.reduce_max(rc_probabilities[:,1:], axis=-1)
    rc_loss = - tf.math.log(second_pre)
